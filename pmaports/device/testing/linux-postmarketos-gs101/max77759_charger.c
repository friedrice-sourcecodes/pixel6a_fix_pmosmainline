...
#define CHGIN_RECOVERY_FALLBACK_UA		3000000
...
static char *max77759_charger_supplied_to[] = {
	"max170xx_battery",
};
...
static int max77759_charger_init(struct max77759_charger *chg)
{
	...
	/* Select the wired CHGIN input and disable the unused wireless input. */
	ret = regmap_update_bits(chg->regmap, MAX77759_CHGR_REG_CHG_CNFG_12,
				 MAX77759_CHGR_REG_CHG_CNFG_12_WCINSEL |
				 MAX77759_CHGR_REG_CHG_CNFG_12_CHGINSEL,
				 MAX77759_CHGR_REG_CHG_CNFG_12_CHGINSEL);
	if (ret)
		goto relock;
...
static void psy_work_item(struct work_struct *work)
{
	...
	if (online.intval) {
		if (!current_limit.intval) {
			current_limit.intval = CHGIN_RECOVERY_FALLBACK_UA;
			dev_info_once(chg->dev,
				      "TCPM supplied no current limit; using %d uA fallback\n",
				      current_limit.intval);
		}
...
static int max77759_charger_probe(struct platform_device *pdev)
{
	...
	psy_cfg.supplied_to = max77759_charger_supplied_to;
	psy_cfg.num_supplicants = ARRAY_SIZE(max77759_charger_supplied_to);
	...
	/*
	 * The gs101 7.0.6 base MFD exposes two aggregated charger IRQs instead
	 * of the newer per-event IRQ resources expected by this driver.  Keep
	 * charging synchronized through the TCPM power-supply notifier for now;
	 * requesting the incompatible IRQ hierarchy prevents the PMIC probe and
	 * can stall early boot on Bluejay.
	 */
	return 0;
}
